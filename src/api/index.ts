...
export const disabledOverridesLocalStorageKey =
    'module-federation-overrides-disabled';
...
export const mergeModuleMap = (originalMap: IModuleMap, newMap: IModuleMap) => {
...
export const getDefaultMap = () => {
    ...
    return Array.from(selectorElements).reduce(
        (promise: Promise<IModuleMap>, scriptEl: Element) => {
            const textContent: IScriptModuleInfo[] = JSON.parse(
                scriptEl.textContent || '[]'
            );
            const imports = textContent.reduce(
                (imports: Record<string, string>, scriptInfo) => {
            ...
            return Promise.all([promise, nextPromise]).then(
                ([originalMap, newMap]) => mergeModuleMap(originalMap, newMap)
            );
        },
...
export const insertOverrideMap = (
    map: string | IModuleMap,
    id: string,
...
const init = () => {
    ...
    const importMaps = Array.from(
        document.querySelectorAll(`script[type="${moduleMapType}"]`)

...
import { createEmptyModuleMap, fireChangedEvent } from '../../../api';
...
import {
    getDefaultModulesInfo,
    getDisabledOverridesInfo,
    getOverriddenModulesInfo,
    toDomain,
    toFileName,
} from './utils';
...
export const ModuleMapList = () => {
    const [notOverriddenMap, setNotOverriddenMap] = useState<IModuleMap>(() =>
    ...
    useEffect(() => {
        ...
        return () => {
            window.removeEventListener(
                'module-federation-overrides:change',
                updateFn
            );
        };
    }, []);
    const defaultModules = getDefaultModulesInfo(notOverriddenMap, searchName);
    const overriddenModules = getOverriddenModulesInfo(
        notOverriddenMap,
        searchName
    );
    const disabledOverrides = getDisabledOverridesInfo(
        notOverriddenMap,
        searchName
    );
